        ...
        public void CheckSnapshots(List<ProcessSnapshot> current)
        {
            ...
            else if (tmRunning && _tmWasOpen)
            {
                foreach (var kvp in _preTmSnapshot)
                {
                    ...
                    if (kvp.Value.Name.Equals("taskmgr", StringComparison.OrdinalIgnoreCase)) continue;
                    if (kvp.Value.Name.Equals("dwm", StringComparison.OrdinalIgnoreCase)) continue;
                    if (kvp.Value.Name.Equals("conhost", StringComparison.OrdinalIgnoreCase)) continue;
