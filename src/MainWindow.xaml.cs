...
namespace Wraith
{
    public partial class MainWindow : Window
    {
        ...
        private async void CheckForUpdates()
        {
            ...
            if (_updateChecker.IsLatest)
            {
                UpdateStatus.Text = "up to date";
                UpdateStatus.Foreground = new System.Windows.Media.SolidColorBrush(
                    System.Windows.Media.Color.FromRgb(0x3f, 0xb9, 0x50));
            }
            else
            {
                UpdateStatus.Text = $"v{_updateChecker.LatestVersion} available";
                UpdateStatus.Foreground = new System.Windows.Media.SolidColorBrush(
        ...
        {
            if (_updateChecker == null || string.IsNullOrEmpty(_updateChecker.DownloadUrl))
                return;
            ...
            bool ok = await _updateChecker.UpdateAsync((pct, info) =>
            {
                Dispatcher.BeginInvoke(() =>
                {
                    UpdateStatus.Text = $"updating {pct}% {info}";
                });
            });
        ...
        {
            if (SystemProcessFilter.IsSystemProcess(ev.ProcessName))
                return;
        ...
        private string GetEventTag(EtwEventType type)
        {
            return type switch
            {
                ...
                EtwEventType.TmEvasion        => "TM    EVAS  ",
                EtwEventType.ImageLoad        => "DLL   LOAD  ",
                EtwEventType.ImageUnload      => "DLL   UNLOAD",
                EtwEventType.RegistryWrite    => "REG   WRITE ",
                EtwEventType.ClipboardRead    => "CLIP  READ  ",
                EtwEventType.ClipboardWrite   => "CLIP  WRITE ",
                EtwEventType.NamedPipeCreate  => "PIPE  CREAT ",
                EtwEventType.NamedPipeConnect => "PIPE  CONN  ",
                _                              => "????  ?????? "
            };
        }
        ...
        private string GetEventColor(EtwEventType type)
        {
            return type switch
            {
                ...
                EtwEventType.ImageLoad        => "#7ee787",
                EtwEventType.ImageUnload      => "#484f58",
                EtwEventType.RegistryWrite    => "#ffa657",
                EtwEventType.ClipboardRead    => "#bc8cff",
                EtwEventType.ClipboardWrite   => "#bc8cff",
                EtwEventType.NamedPipeCreate  => "#79c0ff",
                EtwEventType.NamedPipeConnect => "#79c0ff",
                _                              => "#8b949e"
            };
        }
        ...
        {
            if (SystemProcessFilter.IsSystemProcess(name))
                return;

            Interlocked.Increment(ref _alertCount);

            if (severity == AlertSeverity.High && _soundEnabled)
            {
        ...
        private void SafeEvaluate()
        {
            ...
            try
            {
                ...
                foreach (var alert in alerts)
                {
                    ...
                    if (alert.Severity == AlertSeverity.High && _soundEnabled)
                    {
                        try { SystemSounds.Exclamation.Play(); } catch { }
                    }
                    Dispatcher.BeginInvoke(() =>
                    {
                        ...
                        if (existing != null)
                        {
                            ...
                            existing.SeverityColor = sevColor;
                            var pi = _tracker.GetProcess(alert.PID);
                            if (pi != null)
                            {
                                existing.DllLoads = pi.DllLoadCount;
                                existing.RegistryWrites = pi.RegistryWriteCount;
                                existing.ClipboardAccess = pi.ClipboardReadCount;
                                existing.PipeCount = pi.NamedPipeCount;
                                existing.FocusEvents = pi.FocusEventCount;
                                existing.FileWrites = pi.LogFileWriteCount;
                                existing.ThreadCount = pi.ThreadCreateCount;
                                existing.ApiCalls = pi.AuditApiCallCount;
                            }
                        }
                        else
                        {
                            ...
                            var pi2 = _tracker.GetProcess(alert.PID);
                            if (pi2 != null)
                            {
                                procVm.DllLoads = pi2.DllLoadCount;
                                procVm.RegistryWrites = pi2.RegistryWriteCount;
                                procVm.ClipboardAccess = pi2.ClipboardReadCount;
                                procVm.PipeCount = pi2.NamedPipeCount;
                                procVm.FocusEvents = pi2.FocusEventCount;
                                procVm.FileWrites = pi2.LogFileWriteCount;
                                procVm.ThreadCount = pi2.ThreadCreateCount;
                                procVm.ApiCalls = pi2.AuditApiCallCount;
                            }
                            _suspicious.Insert(0, procVm);
                        }
        ...
        private void BtnExport_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                sw.WriteLine("================================================================");
                sw.WriteLine("  WRAITH v2.1.0 -- THREAT DETECTION REPORT");
                ...
                foreach (var p in _suspicious)
                {
                    ...
                    sw.WriteLine($"           DLL loads: {p.DllLoads}  Reg writes: {p.RegistryWrites}  Clipboard: {p.ClipboardAccess}  Pipes: {p.PipeCount}");
                }
                sw.WriteLine();
